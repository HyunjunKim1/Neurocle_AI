#region The Code Project Open License (CPOL) 1.02
// Origin Author:
// http://www.codeproject.com/Articles/22776/WPF-DrawTools
// Alex Fr
// 
// Modified:
// suoow2. <suoow.yeo@haesung.net>
// 
// Preamble
...
#endregion The Code Project Open License (CPOL) 1.02
...
// 2012-05-03 suoow2 Added.
// : 생산 요청에 따른 중앙선 그리기 클래스 구현.
// 
namespace Common.Drawing
{
    public enum SkeletonNodeType
    {
        None = 0,               // 해당 케이스 없음
        EndPoint = 1,           // 끝점
        NormalPoint = 2,        // 정상 검사 포인트
        BoundaryPoint = 3,      // 경계 지점
        CrossPoint = 4,         // 교차 지점
        TipPoint = 5,         // 교차 지점
    }
    ...
    public class SkeletonNode
    {
        ...
        public SkeletonNode(byte aLineDirection, Int16Point aPosition, float aMeasureSize, byte aType)
        {
            this.LineDirection = aLineDirection;
            this.Position = aPosition;
            this.MeasureSize = aMeasureSize;
            this.Type = aType;
        }

        public string ToDBString()
        {
            return string.Format("L{0}X{1}Y{2}S{3:F2}T{4}", LineDirection, Position.X, Position.Y, MeasureSize, Type);
        }
    }

    public class GraphicsSkeletonLine : GraphicsBase
    {
        #region Private member variables.
        private byte lineDir;
        private new Int16Rect boundaryRect;
        private float medianSize;
        private byte type;
        private SkeletonNode[] nodes = null;
        #endregion

        #region Constructor
        public GraphicsSkeletonLine()
        {
            // TO DO
            this.lineDir = 0;
            ...
            this.type = 0;
            this.nodes = null;
            this.graphicsRegionType = GraphicsRegionType.CenterLine;
            this.graphicsActualScale = 1.0;
            this.caption = string.Empty;
        }

        public GraphicsSkeletonLine(byte aLineDir, Int16Rect aBoundaryRect, float aMeasureSize,
            byte aType, SkeletonNode[] aNodes, double afActualScale)
        {
            // node의 수는 2 이상이어야 그리기가 가능하다.
            Debug.Assert(aNodes != null && aNodes.Length > 1);
            
            this.lineDir = aLineDir;
            this.boundaryRect = aBoundaryRect;
            this.medianSize = aMeasureSize;
            this.type = aType;
            this.nodes = aNodes;

            this.graphicsRegionType = GraphicsRegionType.CenterLine;
            this.graphicsActualScale = afActualScale;
            this.caption = string.Empty;
        ...
        public void Draw(DrawingContext drawingContext, Point relativePoint, double actualScale)
        {
            ...
            double fDrawSize = Math.Min(5.0 / actualScale, 3.0); // scale 값을 그리기 크기로 활용하도록 한다.
            double fFontSize = FontSize / actualScale;

            if (nodes != null && nodes.Length > 0) // normal case.
            {
                Color drawColor = Colors.Transparent;
                
                for (int nIndex = 0; nIndex < nodes.Length; nIndex++)
                {
                    // Node를 표시하는 Rectangle을 그린다.
                    SkeletonNodeType nodeType = (SkeletonNodeType)nodes[nIndex].Type;
                    if (nodeType == SkeletonNodeType.EndPoint)
                    {
                        //if (nIndex % 2 == 1)
                        //    continue;
                    }
                    ...
                    switch (nodeType)
                    {
                        case SkeletonNodeType.EndPoint:
                        case SkeletonNodeType.BoundaryPoint:
                        case SkeletonNodeType.CrossPoint:
                            drawColor = Colors.Orchid;
                            break;
                        case SkeletonNodeType.NormalPoint:
                            drawColor = Colors.OrangeRed;
                            break;
                        case SkeletonNodeType.TipPoint:
                            drawColor = Colors.Blue;
                            break;
                    }
                    drawingContext.DrawRectangle(new SolidColorBrush(drawColor), null,
                        new Rect(relativePoint.X + nodes[nIndex].Position.X - (fDrawSize / 2),
                                 relativePoint.Y + nodes[nIndex].Position.Y - (fDrawSize / 2),
                                 fDrawSize, fDrawSize));
                    
                    if (graphicsActualScale >= 4.0 || (nodeType == SkeletonNodeType.EndPoint && graphicsActualScale >= 1.0))
                    {
                        FormattedText formattedText = new FormattedText(
                            Convert.ToInt32(nodes[nIndex].MeasureSize).ToString(),
                            CultureInfo.InvariantCulture,
                            FlowDirection.LeftToRight,
                            new Typeface(new FontFamily("Arial"), FontStyles.Normal, FontWeights.Bold, FontStretches.Normal),
                            fFontSize,
                            new SolidColorBrush(GraphicsColors.Green));

                        drawingContext.DrawText(formattedText,
                            new Point(relativePoint.X + nodes[nIndex].Position.X + fDrawSize / 2,
                                      relativePoint.Y + nodes[nIndex].Position.Y - (fDrawSize * 4)));
                    }
                }
        ...
        {
            return new PropertiesGraphicsSkeletonLine(this);
        }

        #region Overrides. (Not-using.)
        ...
        public float MedianSize
        {
            get { return medianSize; }
            set { medianSize = value; }
        }
